...
static int RTOp_ROp_fraction_to_boundary_apply_op(
  ...
  , RTOp_ReductTarget reduct_obj )
{
  ...
  for( k = 0; k < sub_dim; ++k, v0_val += v0_val_s, v1_val += v1_val_s, v2_val += v2_val_s, v3_val += v3_val_s )
  {
    ...
    if ((*v1_val) < 0)
       { alpha_max_ith = (*tau)*((*v0_val)-(*v2_val))/(-(*v1_val)); }
    else
       { alpha_max_ith = (*tau)*((*v3_val)-(*v0_val))/((*v1_val)); }
...
static int RTOp_ROp_fraction_to_boundary_reduct_obj_reinit(
  const struct RTOp_RTOp_vtbl_t* vtbl, const void* obj_data
  ,RTOp_ReductTarget reduct_obj
  )
{
