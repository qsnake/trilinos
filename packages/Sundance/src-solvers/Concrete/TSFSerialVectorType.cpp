...
{
  RCP<const SerialVectorSpace> serDomain 
    = rcp_dynamic_cast<const SerialVectorSpace>(domain.ptr());
  RCP<const SerialVectorSpace> serRange 
    = rcp_dynamic_cast<const SerialVectorSpace>(range.ptr());

  RCP<MatrixFactory<double> > rtn 
    = rcp(new DenseSerialMatrixFactory(serDomain, serRange));
