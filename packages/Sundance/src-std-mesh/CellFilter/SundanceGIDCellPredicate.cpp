...
#include "SundanceGIDCellPredicate.hpp"
...
bool GIDCellPredicate::lessThan(const CellPredicateBase* other) const
{
  ...
  return gids_ < dynamic_cast<const GIDCellPredicate*>(other)->gids_;
}
                                   ...
                                   Array<int>& results) const
{
  ...
  {
    int gid = mesh().mapLIDToGID(cellDim_, cellLID[i]);
    results[i] = gids_.contains(gid);
  }
}

XMLObject GIDCellPredicate::toXML() const 
{
  XMLObject rtn("GIDCellPredicate");
  rtn.addAttribute("gidSet", gids_.toString());
  return rtn;
}
